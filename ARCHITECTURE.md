...
| `exporters/` | PNG, GIF, sprite sheet, texture atlas (JSON), Unity (`.meta`-ready layout), Godot (`.tres`/import hints), Unreal (padded POT sheets), **Aseprite** (`.aseprite`, M20 — indexed binary writer + parser, uncompressed cels for byte-stability), **Wang/blob** (`wang-blob`, M22 — builds the 47-tile blob auto-tile sheet from one base tile by edge/corner carving; emits sheet + JSON bitmask→cell map), **Godot 4 TileSet** (`godot-tileset`, M24 — `.tres` atlas source + per-tile terrain peering bits from the blob masks) and **Tiled** (`tiled-tileset`, M24 — `.tsx` wangset + sample `.tmx`), both reusing `wang_blob.build_blob_sheet`. Registry pattern — new exporters register themselves. |
| `tileset/` | Tileset generation (D-001, M23): `TileSet` service builds a coherent terrain family — a base tile plus N variants that **share the base's edges** so any two abut cleanly and each self-tiles. Reuses the animation-sequence pattern (seed anchoring + palette lock) plus **edge lock** (`tileize.lock_edges_to` → re-quantize) verified with the M22 seam metrics; variants assemble into the 47-tile Wang/blob sheet. `pixelforge tileset`, `POST /api/tileset/generate`. |
...
| `projects/` | Project files (JSON on disk, `store.py`), autosave, session recovery. **Portable bundles** (`bundle.py`, M25, D-001): the single-file `.pforge` workspace archive — a deterministic `ZIP_STORED` of a `manifest.json` + PNG assets (sorted entries, fixed timestamp, verbatim image bytes → byte-stable `save→load→save`), with atomic writes (`atomic_write_bytes`), a schema-version + `migrate_manifest` hook, and an `AutosaveManager` (`recover_latest`). `pixelforge project save\|load\|info`, `POST /api/project/save\|load`. |
...
| `renderer/features/tileset/` | **Tileset** tab (M23, D-001): generate a seam-locked terrain family → `POST /api/tileset/generate`; variant brush picker, a paintable N×N grid rendering the tiles edge-to-edge as one continuous surface, a coherence badge, the 47-tile Wang/blob sheet, and (M24) an **Export for a game engine** block downloading Godot `.tres` / Tiled `.tsx`+`.tmx` zips. Pure `tilesetView.ts` (tested). |
| `renderer/features/projects/` | Project browser, autosave indicator. |
| `renderer/features/project/` | **Project bar** (M25, D-001) in the header: name field, Save/Open a portable `.pforge`, and an unsaved-changes dot. Pure `projectView.ts` (dirty tracking + manifest summary, tested). |
