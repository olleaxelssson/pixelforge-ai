...
| `renderer/features/tileset/` | **Tileset** tab (M23, D-001): generate a seam-locked terrain family → `POST /api/tileset/generate`; variant brush picker, a paintable N×N grid rendering the tiles edge-to-edge as one continuous surface, a coherence badge, and the 47-tile Wang/blob sheet. Pure `tilesetView.ts` (tested). |
