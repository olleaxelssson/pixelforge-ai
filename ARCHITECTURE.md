...
| `qa/` | Pixel QA engine (D-013, M9): deterministic defect detectors (`detectors/`: floating pixels, broken clusters, palette overflow, silhouette, pillow shading, light direction) with safe auto-repair, a deterministic `HeuristicCritic` (reuses D-012), and `QAEngine` (run/repair). Findings drop into `SceneGraph.qa`. Opt-in via `qa_enabled`. |
