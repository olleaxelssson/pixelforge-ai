...
| `generation/` | `GenerationPipeline` orchestrating stages; `backends/` implementing `GenerationBackend` (`flux.py` — fp8/offload/ControlNet on the silhouette map, decisions in torch-free `flux_config.py`; `mock.py`); `prompt_builder.py` (fast path) and `plan_compiler.py` (compiles a `SceneGraph` into prompts when planning is on); `benchmark.py` (M2: times + QA-scores a fixed suite, `pixelforge benchmark`). Golden-image regression in `tests/golden/`. |
...
| `exporters/` | PNG, GIF, sprite sheet, texture atlas (JSON), Unity (`.meta`-ready layout), Godot (`.tres`/import hints), Unreal (padded POT sheets), **Aseprite** (`.aseprite`, M20 — indexed binary writer + parser, uncompressed cels for byte-stability). Registry pattern — new exporters register themselves. |
